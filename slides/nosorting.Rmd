...
Let's discuss testing and performance before overlapping generations, since that case is just a bit
tedious.
...
* Get same results on a Xeon E, but this machine takes 50% longer.
* It'd be great to try my EPYC nodes for the parallel sorting, but that is our HPC and
  I can't get gcc 9 there.  To do that right, I'd have to refactor and use ``tbb::parallel_sort``
  directly.
...
```{r,echo=F,fig.align="center",out.width="4.0in",out.height="2.0in",fig.cap="Performance with recombination. Only running the ``tskit`` sim here."}
...
* Parallel sorts have good performance, but are very resource intensive. It's about 25% efficient,
  which is typical.
* The buffering does nearly as well with one thread as the parallel sort with no recombination and as
  well or better with high recombination rates.
* We are **not** taking full advantage of the buffering!!! The liftover from the buffer to the 
  edge table prior to simplification is unnecessary.  (More on that in a bit.)
...
* For each node, record the first/last index where it is a parent in the edge table. (Linear)
* For all entries ``a`` in ``A`` that left offspring, collect ``(a, first, last)`` into list ``L``. (Linear)
* Sort ``L`` according to ``(birth time[a], first, a)``. (This is probably overly complex.)
* As before, go backwards through new births, now stopping when we hit the last simplification time.
* Co-iterate through the edge table and ``L`` to "stitch" together the remaining edges.

This was the hard part. The design skips all of this automatically if nothing in ``A`` left offspring (non-overlapping generations).
