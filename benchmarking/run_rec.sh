...
do
    ...
    STEPS=`echo "5*$N"|bc -l`
    /usr/bin/time -f "%e %M" -o temp.txt ~/src/fwdpp/examples/edge_buffering --N $N  --nsteps $STEPS --seed $SEED --rho $rho
    python3 numtrees.py
    RESULTS=`cat temp.txt`
    echo fwdpp cppsort "$N $rho $RESULTS" >> neutrality_benchmark_rho.txt
    /usr/bin/time -f "%e %M" -o temp.txt ~/src/fwdpp/examples/edge_buffering --N $N  --nsteps $STEPS --seed $SEED --rho $rho --buffer
    python3 numtrees.py
    RESULTS=`cat temp.txt`
    echo fwdpp buffer "$N $rho $RESULTS" >> neutrality_benchmark_rho.txt
